...
public class KafkaGen {
    ...
    public static void main(String[] args) {
        ...
        try (Producer<String, InputData> producer = new KafkaProducer<>(kafkaProps)) {
            for (int i = 0;; i++) {
                String key = String.valueOf(UUID.randomUUID());
                InputData message = generateFakeData();
    ...
    private static InputData generateFakeData() {
        Faker faker = new Faker();

        return new InputData(String.valueOf(faker.number().numberBetween(1000, 9999)), faker.name().name(), faker.company().name() );
    }
